...
private createEndToEndConfigEvent(value) {
    log.debug("Setting end to end to: ${value}")
    return secure(zwave.configurationV1.configurationSet(parameterNumber: 3, size: 1, scaledConfigurationValue: value))
}
...
def poll() {
    ...
    // workaround to keep polling from being shut off
    cmds ?: null
}
...
def configure() {
    log.debug("Updating Danalock configuration...")
